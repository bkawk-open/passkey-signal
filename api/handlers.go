...
func sanitizeDeviceName(name string) string {
	// Allow only printable ASCII (letters, digits, spaces, common punctuation)
...
func handleAuthBegin(ctx context.Context, db *dynamodb.Client, wa *webauthn.WebAuthn, sms *sns.Client, body string) (events.APIGatewayV2HTTPResponse, error) {
	...
	// TODO: remove hardcoded code after testing
	code := "123456"
	...
	// TODO: restore SMS sending after testing
	log.Printf("DEV MODE: OTP for %s is %s (SMS skipped)", maskPhone(phone), code)
...
func handleInviteRegisterBegin(ctx context.Context, db *dynamodb.Client, wa *webauthn.WebAuthn, body string) (events.APIGatewayV2HTTPResponse, error) {
	...
	// Atomically claim the invite to prevent double-registration race
	if err := updateInviteStatus(ctx, db, req.InviteID, "pending", "registering"); err != nil {
		return jsonResp(409, map[string]string{"error": "invite already used"})
	}
...
func handleInviteRegisterFinish(ctx context.Context, db *dynamodb.Client, wa *webauthn.WebAuthn, body string, headers map[string]string) (events.APIGatewayV2HTTPResponse, error) {
	...
	if err := updateInviteStatus(ctx, db, req.InviteID, "registering", "registered"); err != nil {
...
// -- Device enrolment handlers --

func handleDeviceEnrolBegin(ctx context.Context, db *dynamodb.Client, headers map[string]string) (events.APIGatewayV2HTTPResponse, error) {
	token, err := authenticateRequest(ctx, db, headers)
	if err != nil {
		return jsonResp(401, map[string]string{"error": "unauthorized"})
	}

	// Rate limit: max 3 pending enrolments per user (reuse invite limit pattern)
	count, err := countActiveInvites(ctx, db, token.Phone)
	if err != nil {
		log.Printf("countActiveInvites error: %v", err)
		return jsonResp(500, map[string]string{"error": "db error"})
	...
	if count >= 3 {
		return jsonResp(429, map[string]string{"error": "too many active enrolments"})
	}

	enrolToken := uuid.New().String()
	enrolId := uuid.New().String()
	now := time.Now()

	if err := putEnrolToken(ctx, db, EnrolTokenItem{
		PK:      "ENROL#" + enrolToken,
		SK:      "DATA",
		Phone:   token.Phone,
		UserID:  token.UserID,
		EnrolId: enrolId,
		Status:  "pending",
		TTL:     now.Add(3 * time.Minute).Unix(),
	}); err != nil {
		log.Printf("putEnrolToken error: %v", err)
		return jsonResp(500, map[string]string{"error": "db error"})
	}
...
func handleDeviceEnrolRedeem(ctx context.Context, db *dynamodb.Client, body string) (events.APIGatewayV2HTTPResponse, error) {
	var req struct {
		EnrolToken      string `json:"enrolToken"`
		AgreementPubKey string `json:"agreementPubKey"`
		SigningPubKey   string `json:"signingPubKey"`
		DeviceName      string `json:"deviceName"`
	}
	if err := json.Unmarshal([]byte(body), &req); err != nil || req.EnrolToken == "" || req.AgreementPubKey == "" || req.SigningPubKey == "" {
		return jsonResp(400, map[string]string{"error": "enrolToken, agreementPubKey, and signingPubKey required"})
	}

	// Validate public keys are valid base64url, 65 bytes (uncompressed P-256), and on the curve
	agreeBytes, err := fromb64url(req.AgreementPubKey)
	if err != nil || len(agreeBytes) != 65 || agreeBytes[0] != 0x04 {
		return jsonResp(400, map[string]string{"error": "invalid agreementPubKey"})
	}
	if !elliptic.P256().IsOnCurve(new(big.Int).SetBytes(agreeBytes[1:33]), new(big.Int).SetBytes(agreeBytes[33:65])) {
		return jsonResp(400, map[string]string{"error": "agreementPubKey not on P-256 curve"})
	}
	signBytes, err := fromb64url(req.SigningPubKey)
	if err != nil || len(signBytes) != 65 || signBytes[0] != 0x04 {
		return jsonResp(400, map[string]string{"error": "invalid signingPubKey"})
	}
	if !elliptic.P256().IsOnCurve(new(big.Int).SetBytes(signBytes[1:33]), new(big.Int).SetBytes(signBytes[33:65])) {
		return jsonResp(400, map[string]string{"error": "signingPubKey not on P-256 curve"})
	}

	enrol, err := getEnrolToken(ctx, db, req.EnrolToken)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
	if enrol == nil || enrol.TTL < time.Now().Unix() {
		return jsonResp(404, map[string]string{"error": "enrol token not found or expired"})
	}
	if enrol.Status != "pending" {
		return jsonResp(409, map[string]string{"error": "enrol token already used"})
	}
	...
	// Generate a cryptographic secret the device must present on receive/complete
	enrolSecret, err := generateToken()
	if err != nil {
		return jsonResp(500, map[string]string{"error": "secret generation error"})
	}

	// Atomic status transition pending → redeemed, storing device pub keys + secret
	extra := map[string]types.AttributeValue{
		"AgreementPubKey": &types.AttributeValueMemberS{Value: req.AgreementPubKey},
		"SigningPubKey":   &types.AttributeValueMemberS{Value: req.SigningPubKey},
		"DeviceName":      &types.AttributeValueMemberS{Value: deviceName},
		"EnrolSecret":     &types.AttributeValueMemberS{Value: enrolSecret},
	}
	if err := updateEnrolTokenStatus(ctx, db, enrol.PK, "pending", "redeemed", extra); err != nil {
		log.Printf("updateEnrolTokenStatus error: %v", err)
		return jsonResp(409, map[string]string{"error": "enrol token already used"})
	}

	return jsonResp(200, map[string]interface{}{
		"enrolId":     enrol.EnrolId,
		"enrolSecret": enrolSecret,
		"phone":       maskPhone(enrol.Phone),
	})
}

func handleDeviceEnrolDeliver(ctx context.Context, db *dynamodb.Client, headers map[string]string, body string) (events.APIGatewayV2HTTPResponse, error) {
	token, err := authenticateRequest(ctx, db, headers)
	if err != nil {
		return jsonResp(401, map[string]string{"error": "unauthorized"})
	}
	...
	var req struct {
		...
		EncryptedMasterKey string `json:"encryptedMasterKey"`
		WebEphemeralPubKey string `json:"webEphemeralPubKey"`
	}
	if err := json.Unmarshal([]byte(body), &req); err != nil || req.EnrolId == "" || req.EncryptedMasterKey == "" || req.WebEphemeralPubKey == "" {
		return jsonResp(400, map[string]string{"error": "enrolId, encryptedMasterKey, and webEphemeralPubKey required"})
	}

	if len(req.EncryptedMasterKey) > 500 {
		return jsonResp(400, map[string]string{"error": "encryptedMasterKey too large"})
	}

	enrol, err := getEnrolTokenByEnrolId(ctx, db, req.EnrolId)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
	if enrol == nil || enrol.TTL < time.Now().Unix() {
		return jsonResp(404, map[string]string{"error": "enrolment not found or expired"})
	}
	if enrol.Phone != token.Phone {
		return jsonResp(403, map[string]string{"error": "forbidden"})
	}
	if enrol.Status != "redeemed" {
		return jsonResp(409, map[string]string{"error": "enrolment not in redeemed state"})
	}

	extra := map[string]types.AttributeValue{
		"EncryptedMasterKey": &types.AttributeValueMemberS{Value: req.EncryptedMasterKey},
		"WebEphemeralPubKey": &types.AttributeValueMemberS{Value: req.WebEphemeralPubKey},
	}
	if err := updateEnrolTokenStatus(ctx, db, enrol.PK, "redeemed", "delivered", extra); err != nil {
		log.Printf("updateEnrolTokenStatus deliver error: %v", err)
		return jsonResp(409, map[string]string{"error": "invalid status transition"})
	}

	return jsonResp(200, map[string]string{"status": "delivered"})
}

func handleDeviceEnrolReceive(ctx context.Context, db *dynamodb.Client, queryParams map[string]string) (events.APIGatewayV2HTTPResponse, error) {
	enrolId := queryParams["enrolId"]
	enrolSecret := queryParams["enrolSecret"]
	if enrolId == "" || enrolSecret == "" {
		return jsonResp(400, map[string]string{"error": "enrolId and enrolSecret required"})
	}

	enrol, err := getEnrolTokenByEnrolId(ctx, db, enrolId)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
	if enrol == nil || enrol.TTL < time.Now().Unix() {
	...
	// Verify the caller is the device that redeemed this enrolment
	if subtle.ConstantTimeCompare([]byte(enrol.EnrolSecret), []byte(enrolSecret)) != 1 {
		return jsonResp(403, map[string]string{"error": "forbidden"})
	}

	if enrol.Status == "delivered" || enrol.Status == "completed" {
		return jsonResp(200, map[string]interface{}{
			"status":             enrol.Status,
			"encryptedMasterKey": enrol.EncryptedMasterKey,
			"webEphemeralPubKey": enrol.WebEphemeralPubKey,
		})
	}
...
func handleDeviceEnrolComplete(ctx context.Context, db *dynamodb.Client, body string) (events.APIGatewayV2HTTPResponse, error) {
	var req struct {
		EnrolId     string `json:"enrolId"`
		DeviceId    string `json:"deviceId"`
		EnrolSecret string `json:"enrolSecret"`
	}
	if err := json.Unmarshal([]byte(body), &req); err != nil || req.EnrolId == "" || req.DeviceId == "" || req.EnrolSecret == "" {
		return jsonResp(400, map[string]string{"error": "enrolId, deviceId, and enrolSecret required"})
	}
	...
	if enrol.Status != "delivered" {
		return jsonResp(409, map[string]string{"error": "enrolment not in delivered state"})
	}

	// Verify the caller is the device that redeemed this enrolment
	if subtle.ConstantTimeCompare([]byte(enrol.EnrolSecret), []byte(req.EnrolSecret)) != 1 {
		return jsonResp(403, map[string]string{"error": "forbidden"})
	}

	// Enforce max 10 devices per user
	existingDevices, err := getUserDevices(ctx, db, enrol.Phone)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
	if len(existingDevices) >= 10 {
		return jsonResp(429, map[string]string{"error": "too many devices"})
	}

	// Always use the keys established at redeem time — never allow override
	if err := putDevice(ctx, db, DeviceItem{
		PK:              "USER#" + enrol.Phone,
		SK:              "DEVICE#" + req.DeviceId,
		DeviceId:        req.DeviceId,
		DeviceName:      enrol.DeviceName,
		AgreementPubKey: enrol.AgreementPubKey,
		SigningPubKey:   enrol.SigningPubKey,
		CreatedAt:       time.Now().UTC().Format(time.RFC3339),
	}); err != nil {
		log.Printf("putDevice error: %v", err)
		return jsonResp(500, map[string]string{"error": "db error"})
	}

	// Mark enrol as completed
	if err := updateEnrolTokenStatus(ctx, db, enrol.PK, "delivered", "completed", nil); err != nil {
		log.Printf("updateEnrolTokenStatus complete error: %v", err)
	}

	return jsonResp(200, map[string]string{"status": "completed"})
}

func handleDeviceEnrolStatus(ctx context.Context, db *dynamodb.Client, headers map[string]string, queryParams map[string]string) (events.APIGatewayV2HTTPResponse, error) {
	token, err := authenticateRequest(ctx, db, headers)
	if err != nil {
		return jsonResp(401, map[string]string{"error": "unauthorized"})
	}
	...
	enrolId := queryParams["enrolId"]
	if enrolId == "" {
		return jsonResp(400, map[string]string{"error": "enrolId required"})
	}
	...
	if enrol == nil {
		return jsonResp(404, map[string]string{"error": "enrolment not found"})
	}
	if enrol.Phone != token.Phone {
		return jsonResp(403, map[string]string{"error": "forbidden"})
	}

	expired := enrol.TTL < time.Now().Unix()
	...
	// Include device's agreement pub key when redeemed (for web ECDH)
	if enrol.Status == "redeemed" && enrol.AgreementPubKey != "" {
		resp["agreementPubKey"] = enrol.AgreementPubKey
	}

	return jsonResp(200, resp)
}

func handleDeviceAuth(ctx context.Context, db *dynamodb.Client, body string) (events.APIGatewayV2HTTPResponse, error) {
	var req struct {
		DeviceId string `json:"deviceId"`
	}
	if err := json.Unmarshal([]byte(body), &req); err != nil || req.DeviceId == "" {
		return jsonResp(400, map[string]string{"error": "deviceId required"})
	}

	device, err := getDeviceByDeviceId(ctx, db, req.DeviceId)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
	if device == nil {
		return jsonResp(404, map[string]string{"error": "device not found"})
	}

	// Generate 32-byte random challenge
	challengeBytes := make([]byte, 32)
	if _, err := rand.Read(challengeBytes); err != nil {
		return jsonResp(500, map[string]string{"error": "challenge generation error"})
	}
	challenge := b64url(challengeBytes)
	challengeId := uuid.New().String()

	// Extract phone from PK (USER#<phone>)
	phone := strings.TrimPrefix(device.PK, "USER#")

	if err := putDeviceChallenge(ctx, db, DeviceChallengeItem{
		PK:        "DEVCHALLENGE#" + challengeId,
		SK:        "DATA",
		DeviceId:  req.DeviceId,
		Phone:     phone,
		Challenge: challenge,
...
func handleDeviceVerify(ctx context.Context, db *dynamodb.Client, body string) (events.APIGatewayV2HTTPResponse, error) {
	var req struct {
		ChallengeId string `json:"challengeId"`
		DeviceId    string `json:"deviceId"`
		Signature   string `json:"signature"`
	}
	if err := json.Unmarshal([]byte(body), &req); err != nil || req.ChallengeId == "" || req.DeviceId == "" || req.Signature == "" {
		return jsonResp(400, map[string]string{"error": "challengeId, deviceId, and signature required"})
	}

	// Atomically consume challenge
	challenge, err := consumeDeviceChallenge(ctx, db, req.ChallengeId)
	if err != nil || challenge == nil {
		return jsonResp(400, map[string]string{"error": "invalid or expired challenge"})
	}
	if challenge.TTL < time.Now().Unix() {
		return jsonResp(400, map[string]string{"error": "challenge expired"})
	}
	if challenge.DeviceId != req.DeviceId {
		return jsonResp(403, map[string]string{"error": "device mismatch"})
	}

	// Look up device to get signing public key
	device, err := getDeviceByDeviceId(ctx, db, req.DeviceId)
	...
	sigBytes, err := fromb64url(req.Signature)
	if err != nil {
		return jsonResp(400, map[string]string{"error": "invalid signature encoding"})
	}
	pubKeyBytes, err := fromb64url(device.SigningPubKey)
	if err != nil || len(pubKeyBytes) != 65 {
		return jsonResp(500, map[string]string{"error": "invalid stored public key"})
	}

	x := new(big.Int).SetBytes(pubKeyBytes[1:33])
	y := new(big.Int).SetBytes(pubKeyBytes[33:65])
	if !elliptic.P256().IsOnCurve(x, y) {
		return jsonResp(500, map[string]string{"error": "stored public key not on curve"})
	}
	...
	challengeRaw, err := fromb64url(challenge.Challenge)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "challenge decode error"})
	}
	hash := sha256.Sum256(challengeRaw)

	if !ecdsa.VerifyASN1(pubKey, hash[:], sigBytes) {
		return jsonResp(401, map[string]string{"error": "signature verification failed"})
	}
	...
	user, err := getUser(ctx, db, phone)
	if err != nil || user == nil {
		return jsonResp(500, map[string]string{"error": "user not found"})
	}
...
func handleDeleteDevice(ctx context.Context, db *dynamodb.Client, headers map[string]string, body string) (events.APIGatewayV2HTTPResponse, error) {
	token, err := authenticateRequest(ctx, db, headers)
	if err != nil {
		return jsonResp(401, map[string]string{"error": "unauthorized"})
	}
	...
	devicePhone := strings.TrimPrefix(device.PK, "USER#")
	if devicePhone != token.Phone {
		return jsonResp(403, map[string]string{"error": "forbidden"})
	}

	if err := deleteDevice(ctx, db, token.Phone, req.DeviceId); err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
...
func handleListDevices(ctx context.Context, db *dynamodb.Client, headers map[string]string) (events.APIGatewayV2HTTPResponse, error) {
	token, err := authenticateRequest(ctx, db, headers)
	if err != nil {
		return jsonResp(401, map[string]string{"error": "unauthorized"})
	}

	devices, err := getUserDevices(ctx, db, token.Phone)
	if err != nil {
		return jsonResp(500, map[string]string{"error": "db error"})
	}
