(function () {
    ...
    const API = 'https://api.passkey-signal.bkawk.com';
    const {
        startRegistration,
    ...
    const $dkgView = document.getElementById('dkgView');
    const $dkgStatus = document.getElementById('dkgStatus');
    ...
    // -- ECDH crypto helpers for iOS device enrolment --

    async function generateEphemeralP256() {
        return crypto.subtle.generateKey(
    ...
    async function ecdhDeriveKey(privateKey, peerPublicKeyRaw) {
        var peerPubKey = await crypto.subtle.importKey(
        ...
        return crypto.subtle.deriveKey(
            {
                name: 'HKDF',
                hash: 'SHA-256',
                salt: new TextEncoder().encode('passkey-ios-device-enrol-v1'),
    ...
    async function ecdhEncryptMasterKey(mk, peerPubKeyRaw, ephemeralKeyPair) {
        var aesKey = await ecdhDeriveKey(ephemeralKeyPair.privateKey, peerPubKeyRaw);
        var iv = crypto.getRandomValues(new Uint8Array(12));
        var rawKey = await crypto.subtle.exportKey('raw', mk);
        var ciphertext = await crypto.subtle.encrypt(
            { name: 'AES-GCM', iv: iv },
        ...
        combined.set(iv, 0);
        combined.set(new Uint8Array(ciphertext), 12);
        var ephPubRaw = await crypto.subtle.exportKey('raw', ephemeralKeyPair.publicKey);
        return {
            encryptedMasterKey: bufferToB64url(combined.buffer),
            webEphemeralPubKey: bufferToB64url(ephPubRaw),
        };
    }
    ...
    function setState(newState, ctx) {
        ...
        switch (newState) {
            ...
            case State.ERROR:
                ...
                break;
            case State.DKG_SETUP:
                $dkgView.classList.remove('hidden');
                $dkgStatus.textContent = ctx.message || 'Setting up your wallet...';
                break;
    ...
    function renderPasskeySection() {
        ...
        if (masterKey) {
            ...
            var linkAppBtn = document.createElement('button');
            linkAppBtn.className = 'btn-primary';
            linkAppBtn.textContent = 'Link Mobile App';
            linkAppBtn.addEventListener('click', handleLinkMobileApp);
            btnRow.appendChild(linkAppBtn);
        }
        ...
        $passkeySection.appendChild(deviceListContainer);
    ...
    // -- Device enrolment (iOS app) --

    var deviceEnrolPollTimer = null;
    var deviceEnrolCountdownTimer = null;

    async function handleLinkMobileApp() {
        if (!masterKey) return;
        var token = localStorage.getItem('passkey_token');
        var $ps = document.getElementById('passkeyStatus');
        try {
            $ps.textContent = 'Creating enrolment...';

            var resp = await api('/device/enrol/begin', {
                method: 'POST',
                headers: { Authorization: 'Bearer ' + token },
            });

            var enrolToken = resp.enrolToken;
            var enrolId = resp.enrolId;
            var enrolUrl = 'https://passkey-signal.bkawk.com/enrol?token=' + enrolToken;

            $ps.textContent = '';
            showDeviceEnrolModal(enrolId, enrolUrl, token);
        } catch (e) {
            console.error('Failed to create device enrolment:', e);
            $ps.textContent = e.message === 'too many active enrolments' ?
                'Too many active enrolments. Wait for them to expire.' :
                'Failed to create enrolment. Try again.';
        }
    }

    function showDeviceEnrolModal(enrolId, enrolUrl, token) {
        var existing = document.querySelector('.invite-overlay');
        if (existing) existing.remove();
        ...
        var title = document.createElement('h2');
        title.textContent = 'Link Mobile App';

        var desc = document.createElement('p');
        desc.style.fontSize = '0.9em';
        desc.style.color = '#666';
        desc.textContent = 'Scan this QR code with the iOS app to link your device.';
        ...
        var countdown = document.createElement('div');
        countdown.className = 'invite-countdown';
        countdown.id = 'deviceEnrolCountdown';

        var statusEl = document.createElement('div');
        statusEl.className = 'invite-status';
        statusEl.id = 'deviceEnrolStatus';
        statusEl.textContent = 'Waiting for device to scan...';
        ...
        cancelBtn.textContent = 'Cancel';
        cancelBtn.addEventListener('click', function () {
            clearInterval(deviceEnrolPollTimer);
            clearInterval(deviceEnrolCountdownTimer);
            deviceEnrolPollTimer = null;
            deviceEnrolCountdownTimer = null;
            overlay.remove();
        });
        ...
        function updateCountdown() {
            ...
            var mins = Math.floor(remaining / 60000);
            var secs = Math.floor((remaining % 60000) / 1000);
            var $cd = document.getElementById('deviceEnrolCountdown');
            if ($cd) $cd.textContent = mins + ':' + (secs < 10 ? '0' : '') + secs;
            if (remaining <= 0) {
                clearInterval(deviceEnrolCountdownTimer);
                clearInterval(deviceEnrolPollTimer);
                var $ds = document.getElementById('deviceEnrolStatus');
                if ($ds) $ds.textContent = 'Expired';
            }
        }
        updateCountdown();
        deviceEnrolCountdownTimer = setInterval(updateCountdown, 1000);

        // Poll for status changes
        var ephemeralKeyPair = null;

        deviceEnrolPollTimer = setInterval(async function () {
            try {
                var data = await api('/device/enrol/status?enrolId=' + encodeURIComponent(enrolId), {
                    method: 'GET',
                    headers: { Authorization: 'Bearer ' + token },
                });
                var $ds = document.getElementById('deviceEnrolStatus');
                ...
                if (data.expired) {
                    $ds.textContent = 'Expired';
                    clearInterval(deviceEnrolPollTimer);
                    clearInterval(deviceEnrolCountdownTimer);
                    return;
                }

                if (data.status === 'redeemed' && data.agreementPubKey && !ephemeralKeyPair) {
                    // Device submitted pub keys — do ECDH and deliver encrypted master key
                    $ds.textContent = 'Encrypting master key...';
                    ephemeralKeyPair = await generateEphemeralP256();
                    var devicePubKeyRaw = new Uint8Array(b64urlToBuffer(data.agreementPubKey));
                    var encrypted = await ecdhEncryptMasterKey(masterKey, devicePubKeyRaw, ephemeralKeyPair);

                    await api('/device/enrol/deliver', {
                        method: 'POST',
                        headers: { Authorization: 'Bearer ' + token },
                        body: JSON.stringify({
                            enrolId: enrolId,
                            encryptedMasterKey: encrypted.encryptedMasterKey,
                            webEphemeralPubKey: encrypted.webEphemeralPubKey,
                        }),
                    });
                    $ds.textContent = 'Key delivered. Waiting for device...';
                }

                if (data.status === 'completed') {
                    $ds.textContent = 'Device linked!';
                    clearInterval(deviceEnrolPollTimer);
                    clearInterval(deviceEnrolCountdownTimer);
                    setTimeout(function () {
                        overlay.remove();
                ...
                console.error('Device enrol poll error:', e);
            }
        }, 2000);
    }

    // -- Device list (iOS devices) --
    ...
    async function loadDeviceList() {
        ...
        try {
            ...
            heading.textContent = 'Linked Devices';
            ...
            for (var i = 0; i < devices.length; i++) {
                ...
                nameEl.className = 'passkey-id';
                nameEl.textContent = dev.deviceName || dev.deviceId.substring(0, 16) + '...';

                var dateEl = document.createElement('div');
                dateEl.className = 'passkey-date';
                if (dev.createdAt) {
                    var d = new Date(dev.createdAt);
                    dateEl.textContent = 'Linked ' + d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                }
                ...
                delBtn.setAttribute('data-device-id', dev.deviceId);
                delBtn.addEventListener('click', function () {
                    handleRemoveDevice(this.getAttribute('data-device-id'));
                });
                actions.appendChild(delBtn);
    ...
    async function handleRemoveDevice(deviceId) {
        if (!confirm('Remove this device? It will no longer be able to access your account.')) return;
        var token = localStorage.getItem('passkey_token');
        var $ps = document.getElementById('passkeyStatus');
        try {
            $ps.textContent = 'Removing device...';
            await api('/device', {
                method: 'DELETE',
                headers: { Authorization: 'Bearer ' + token },
            ...
            $ps.textContent = 'Remove failed. Try again.';
        }
    }
    ...
    async function handleReauth() {
        ...
        try {
            ...
            // Use the re-auth token for subsequent API calls
            var token = result.token || localStorage.getItem('passkey_token');

            if (!prfResult) {
                console.warn('PRF not available from assertion');
                // PRF not supported — still unlock notes with master key if possible
            }

            if (!result.credentialId) {
                console.warn('No credentialId in login response');
                $rs.textContent = 'Login succeeded but no credential ID returned.';
                return;
            }
            ...
            // --- Load wallet data from note if available ---
            if (!walletData && masterKey) {
                try {
                    var noteData = await api('/note', {
                        method: 'GET',
                        headers: { Authorization: 'Bearer ' + token },
                    });
                    if (noteData.exists) {
                        var noteText = await decryptNote(masterKey, noteData.ciphertext, noteData.iv);
                        walletData = JSON.parse(noteText);
                        console.log('Wallet loaded from note:', walletData);
                    }
                } catch (loadErr) {
                    console.error('Failed to load wallet from note:', loadErr);
                }
            }

            // --- DKG: Generate wallet if not already done ---
            if (!walletData && prfResult) {
                try {
            ...
            // Logout the re-auth token (keep the original session token)
            if (result.token) {
                api('/logout', {
                    method: 'POST',
    ...
    function renderNoteSection() {
        ...
        // Show wallet seed status if DKG has completed
        if (walletData) {
            var walletDiv = document.createElement('div');
            walletDiv.className = 'wallet-section';

            var walletHeading = document.createElement('h3');
    ...
    async function handleOTPVerify() {
        ...
        try {
            ...
            $verifyOtpBtn.disabled = false;
            $verifyOtpBtn.textContent = 'Verify';
            $otpStatus.textContent = 'Verification failed. Please check the code and try again.';
            $otpStatus.className = 'error';
        }
    }
    ...
    async function handleResendOTP() {
        ...
        try {
            ...
            $otpStatus.textContent = 'Failed to resend. Please try again.';
            $otpStatus.className = 'error';
        }
    }
    ...
    async function handleContinue() {
        var phone = $phone.value.trim().replace(/[\s\-()]/g, '');
        if (!phone || !/^\d{6,15}$/.test(phone)) {
            setState(State.ERROR, { message: 'Enter a valid phone number (digits only).' });
            return;
        }
    ...
    (async function () {
        ...
        if (inviteId && location.hash.length > 1) {
            // Validate invite ID format (UUID) and secret (base64url, reasonable length)
            var inviteSecretB64 = location.hash.substring(1);
            if (/^[0-9a-f\-]{36}$/.test(inviteId) && inviteSecretB64.length > 0 && inviteSecretB64.length <= 100) {
